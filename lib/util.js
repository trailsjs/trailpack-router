...
module.exports = {
   ...
   * same.
   */
  findRoutePathConflicts (operandRoute, routeList) {
    return _.filter(routeList, route => {
      const methodIntersection = _.intersection(
        _.flatten([ route.method ]), _.flatten([ operandRoute.method ])
      )
      const isMethodOverlap = methodIntersection.length > 0
      const isPathIdentical = operandRoute.path == route.path
  ...
  mergeRoutes (routes, overrides) {
    const filteredRoutes = _.filter(routes, route => {
      //const pathDupes = _.filter(overrides


      //console.log('route', route, 'dupes', dupes)
    })
