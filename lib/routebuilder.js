...
module.exports = {
  ...
  buildFootprintRoutes (config, api) {
    ...
    const controllerFootprintRoutes = lib.Footprints.getControllerRoutes(config, api)
  ...
  buildCustomRoutes (config, footprintRoutes) {
    return lib.RouteBuilder.mergeCustomRoutes(config.routes, footprintRoutes)
  },
  ...
  buildRoutesWithPrerequisites (config, api, allRoutes) {
    return allRoutes.map(route => {
      return _.merge(route, lib.RouteBuilder.getRoutePrerequisites(config.policies, route))
    })
  },
  ...
  getFootprintHandlerRoutes (config, api) {
    return lib.RouteBuilder.getControllerHandlers(api.controllers).map(handler => {
      return {
        method: '*',
        path: lib.Footprints.getControllerPath(handler, config),
        handler: lib.Footprints.getRouteHandler(handler),
        config: {
          id: lib.Footprints.getRouteIdFromHandler(handler, 'footprint', config)
        }
      }
    })
  },
