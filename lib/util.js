...
const isClassStandardized = {} // cache to know if class is standar or not
...
module.exports = {
  ...
  buildRoute (app, rawRoute) {
    ...
    if (_.isString(route.handler)){
      ...
      route.handler.isStandardized = lib.Util.isStandardized(_.get(app.controllers, rawRoute.handler.split('.')[0]))
    }
  ...
  isStandardized: obj => {
    ...
    if (isClassStandardized[className]) {
      return isClassStandardized[className]
    }

    if (className == Controller.name || className == Policy.name) {
      return isClassStandardized[className] = true
    }
    else if (className == 'Object') {
      return isClassStandardized[className] = false
    }
    else {
      return isClassStandardized[className] = lib.Util.isStandardized(obj.__proto__)
    }
  },
    ...
    return _.defaultsDeep({}, route, {
      config: {
        pre: _.compact(_.map(prerequisites, policy => {
          ...
          pre.isStandardized = lib.Util.isStandardized(_.get(app.policies, policy.split('.')[0]))
          if (!pre) {
  ...
  getRoutePrerequisites (policiesConfig, route) {
    if (_.isString(route.handler)){
      ...
      let handlerPolicies = _.get(policiesConfig, [controller, handler])

      if (!handlerPolicies) {
        const globControllerPolicies = _.get(policiesConfig, [controller, '*'])
        const globPolicies = _.get(policiesConfig, ['*'])
        if (globControllerPolicies) {
          handlerPolicies = globControllerPolicies
        }
        else if (globPolicies) {
          handlerPolicies = globPolicies
        }
      else {
          handlerPolicies = []
        }
      ...
      //no prerequisites for other handler types
      return []
    }
  },
